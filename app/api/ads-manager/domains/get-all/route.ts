...
export const GET = async (request: NextRequest) => {
  const params: GetAllDomainManagerServiceProps =
  ...
  const queryString = `SELECT * FROM v_Domains ${conditionQuery} LIMIT ? OFFSET ?`;
  ...
  try {
    ...
    const rows: any = await query({
      query: `SELECT COUNT(*) AS total_count FROM v_Domains ${conditionQuery}`,
