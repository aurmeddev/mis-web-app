...
import { UpdateApProfileBrandPermissionsProps } from "@/lib/features/users/permissions/type/UserPermissionsProps";
...
export const PUT = async (request: NextRequest) => {
  ...
  const { ap_profile_id, brand_id }: UpdateApProfileBrandPermissionsProps =
    await request.json();
  ...
  const { isSuccess, data, message } =
    await user.updateApProfileBrandPermissions({
      ap_profile_id,
      brand_id,
    });
