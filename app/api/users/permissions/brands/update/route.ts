...
import { UpdateUserBrandPermissionsProps } from "@/lib/features/users/permissions/type/UserPermissionsProps";
...
export const PUT = async (request: NextRequest) => {
  ...
  const { user_id, brand_id }: UpdateUserBrandPermissionsProps =
    await request.json();
  ...
  const { isSuccess, data, message } = await user.updateUserBrandPermissions({
